...
// Internal helper: Loads a file from a resolved path (provided by ThemeLoader)
bool Manager::load_icon_image(const std::string &path,
                              const std::string &cache_key, double size) {

  // Logic for Raster Images (PNG/JPG)
  ...
  if (path.find(".svg") != std::string::npos) {
    ...
    // Convert RGBA to PRGB32 for Blend2D
    for (int iy = 0; iy < h; ++iy) {
      ...
      for (int ix = 0; ix < w; ++ix) {
        ...
        if (a > 0) {
          ...
          g = (g * a + 127) / 255;
          b = (b * a + 127) / 255;
          dstRow[ix * 4 + 0] = (unsigned char)b; // Blue
          dstRow[ix * 4 + 1] = (unsigned char)g; // Green
          dstRow[ix * 4 + 2] = (unsigned char)r; // Red
          dstRow[ix * 4 + 3] = (unsigned char)a; // Alpha
        } else {
...
void Manager::render_icon(BLContext &ctx, const std::string &icon_name,
  ...
  // 2. Resolve Name -> Path via Theme
  // We simply pass whatever icon_name we received to the theme loader.
  // If it's a valid name in the theme, we get a path. If not, we get empty.
  std::string path = theme_loader.lookup_icon(icon_name);
  if (path.empty()) {
    // Icon not found in theme, skip rendering
    return;
  }

  // 3. Load the resolved file
